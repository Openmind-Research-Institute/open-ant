...
from step_stats import StepStatsLogger
...
class ForwardTask:
    def __init__(self, action_cost_weight=0.0):
        ...
        self.observation_space = spaces.Box(
            low=-1.5, high=1.5, shape=(24,), dtype=np.float32
        )
    ...
    def __call__(self, info, action):
        ...
        cost_action = (
            np.sum(np.square(self.last_action - action)) * self.action_cost_weight
        )
        self.last_pos = pos
        self.previous_pos_timestamp = pos_timestamp
...
class BackAndForthTask:
    ...
    def __call__(self, info, action):
        ...
        if (
            np.dot(pos - self.origin, self.reward_direction_I) > 0
            and np.linalg.norm(pos - self.origin) > self.radius
        ):
        ...
        reward_direction_B = np.array(
            [
                np.dot(info["reward_direction_I"], info["heading_vector"]),
...
class EmbodiedAnt(gym.Env):
    ...
    def step(self, action, sleep_until_next_step=True):
        ...
        time_between_steps = (
            step_start - self.last_step_time if self.last_step_time is not None else 0.0
        )
        ...
        for i in range(4):
            action[2 * i] = (
                np.clip(action[2 * i], -1, 1) * self.joint_config["hip_range"]
                + self.joint_config["hip_zero"]
            )
            action[2 * i + 1] = (
                np.clip(action[2 * i + 1], -1, 1) * self.joint_config["knee_range"]
                + self.joint_config["knee_zero"]
            )
        t0 = time.time()
        self.motor_controller.set_positions(action)
        set_positions_duration = time.time() - t0
        ...
        if self.last_step_time is not None:
            ...
            if sleep_duration < 0:
                ...
                print(
                    f"Warning: calls to step() exceeded step size (time since last step: {time_since_last_step:.3f}s)."
                )
                sleep_duration = 0
        ...
        info = self.get_observation()
        get_observation_duration = time.time() - time_start
        info["env_time_to_get_obs"] = get_observation_duration

        now = time.time()
        obs_age_image = now - info["position_timestamp"]
        obs_age_imu = now - info.get("timestamp", 0.0)
        ...
        if len(errors) > 0:  # only log errors if there are any
            self.error_log.write(
                f"{time.time()}, " + ", ".join(map(str, errors)) + "\n"
            )
            self.error_log.flush()
        check_errors_duration = time.time() - time_start
        info["env_time_to_check_errors"] = check_errors_duration
        ...
        if self.render_mode == "human" or self.render_mode == "rgb_array":
            ...
            if isinstance(self.task, BackAndForthTask):
                ...
                self.tracker.draw_circle(self.vis_frame, origin_3D_O, self.task.radius)
                reward_direction_I = np.array(
                    [
                        self.task.reward_direction_I[0],
                        self.task.reward_direction_I[1],
                        0.0,
                    ]
                )
                self.tracker.draw_arrow(self.vis_frame, origin_3D_O, reward_direction_I)
                r_B = np.array(
                    [info["reward_direction_B_x"], info["reward_direction_B_y"], 0.0]
                )
                if "body" in info["bodies"]:
                    R_B_I = info["bodies"]["body"]["orientation"]
                    r_I = R_B_I @ r_B
                    self.tracker.draw_arrow(self.vis_frame, self.last_pos, r_I)
            info["vis_frame"] = self.vis_frame
            if self.render_mode == "human":
                if self.i % 10 == 0:
                    show_image(self.vis_frame)
            render_duration = time.time() - time_start
        ...
        self.stats.record(
            ...
            set_positions_duration=set_positions_duration,
            get_observation_duration=get_observation_duration,
            check_errors_duration=check_errors_duration,
            render_duration=render_duration,
            obs_age_image=obs_age_image,
            obs_age_imu=obs_age_imu,
        )
    ...
    def get_observation(self):
        ...
        if "body" in bodies:
            ...
            info["current_x_position"] = (
                self.last_pos[0] if self.last_pos is not None else 0.0
            )
            info["current_y_position"] = (
                self.last_pos[1] if self.last_pos is not None else 0.0
            )
            heading_vector = self.last_heading_vector
            info["position_timestamp"] = (
                self.last_position_timestamp
                if self.last_position_timestamp is not None
                else 0.0
            )
        ...
        joint_positions, joint_velocities, joint_loads = (
            self.motor_controller.get_feedback()
        )
...
def make_ant_env(cfg, **kwargs):
    motor_controller = MotorController(
        port=cfg["motor_port"], motor_list=cfg["motor_list"]
    )
    ...
    return EmbodiedAnt(
        motor_controller=motor_controller, imu=imu, tracker=tracker, **kwargs
    )
...
if __name__ == "__main__":
    ...
    env = EmbodiedAnt(
        motor_controller=motor_controller, imu=imu, tracker=tracker, dt=0.05
    )
    i = 0
