...
class SAC:
    ...
    def get_action(self, obs, evaluate=False):
        ...
        # Otherwise, use pi0 if warm_start, or use random actions if not warm_start
        if self.warm_start:
            actions, _, _ = self.actor.get_action(torch.Tensor(self.obs).to(self.device))
...
def parse_args():
    ...
    parser.add_argument("--warm_start", action="store_true", default=False,
                    help="trick 2: reset step to 0 and use pi0 during warmup instead of random actions")
...
if __name__ == "__main__":
    ...
    if args.weights_path is not None:
        ...
        if not args.warm_start:  # warm start means don't load sim1's buffer
