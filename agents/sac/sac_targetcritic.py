...
class SAC:
    ...
    def agent_step(self, next_obs, actions, rewards, terminations, truncations, infos):
        ...
        alpha_loss = None
        actor_loss = None
        mean_q1 = mean_q2 = mean_td_target = mean_q_actor = None      # ADD
        actor_entropy = log_std_mean = log_std_std = None              # ADD
        ...
        if self.global_step > self.learning_starts:
            ...
            with torch.no_grad():
                ...
                mean_td_target = next_q_value.mean().item()   # ADD
            ...
            qf_loss.backward()
            critic_grad_norm = sum(p.grad.norm().item()**2 for p in list(self.qf1.parameters()) + list(self.qf2.parameters()) if p.grad is not None) ** 0.5
            ...
            if self.global_step % self.policy_frequency == 0:
                for _ in range(
                    ...
                    qf1_pi = self.qf1_target(data["observations"], pi)
                    qf2_pi = self.qf2_target(data["observations"], pi)
                    min_qf_pi = torch.min(qf1_pi, qf2_pi)
                    actor_loss = ((self.alpha * log_pi) - min_qf_pi).mean()

                    # added stats about actor
                    mean_q_actor = min_qf_pi.detach().mean().item()
                    actor_entropy = (-log_pi).detach().mean().item()
                    with torch.no_grad():
                        _, log_std = self.actor(data["observations"])
                        log_std_mean = log_std.mean().item()
                        log_std_std  = log_std.std().item()
