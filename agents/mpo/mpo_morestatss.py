...
class MPO:
    def __init__(self, args, envs, disk_folder='', run_name=None, runs_directory='runs'):
        ...
        self.keys_agent_vars = [
            ...
            'actor_grad_norm_P', 'actor_grad_norm_M', 'actor_grad_norm_S',
            # ← NEW
            'mean_bootstrap_q', 'bellman_residual',
            'mean_delta', 'mean_delta_pos', 'mean_delta_neg',
            'mean_reward_term', 'mean_next_q_term', 'mean_q_k_term',
            'mean_is_coef', 'min_is_coef', 'frac_is_clipped',
    ...
    def _learn(self):
        ...
        # t0 = time.time()
        (loss_q, mean_q, critic_grad_norm, per_critic_means,
         mean_td_target, mean_bootstrap_q, bellman_residual,
         mean_delta, mean_delta_pos, mean_delta_neg,
         mean_reward_term, mean_next_q_term, mean_q_k_term,
         mean_is_coef, min_is_coef, frac_is_clipped) = self._update_critic(data)
 
        # t_critic = time.time() - t0
 
        loss_p = kl_mu = kl_sigma = actor_entropy = actor_grad_norm = 0.0
        actor_grad_norm_P = actor_grad_norm_M = actor_grad_norm_S = 0.0
        ...
        if not self.args.decouple_q_learning:
            # t0 = time.time()
            (loss_p, kl_mu, kl_sigma, actor_entropy, actor_grad_norm,
             actor_grad_norm_P, actor_grad_norm_M, actor_grad_norm_S) = \
                self._m_step(data.observations, action_samples, weights, b_mu, b_sigma)
        ...
        return {
            ...
            'actor_grad_norm_P': actor_grad_norm_P,
            'actor_grad_norm_M': actor_grad_norm_M,
            'actor_grad_norm_S': actor_grad_norm_S,
            **{f'mean_q_{i}': v for i, v in enumerate(per_critic_means)},
            # ← NEW: Retrace decomposition metrics
            'mean_bootstrap_q':  mean_bootstrap_q,
            'bellman_residual':  bellman_residual,
            'mean_delta':        mean_delta,
            'mean_delta_pos':    mean_delta_pos,
            'mean_delta_neg':    mean_delta_neg,
            'mean_reward_term':  mean_reward_term,
            'mean_next_q_term':  mean_next_q_term,
            'mean_q_k_term':     mean_q_k_term,
            'mean_is_coef':      mean_is_coef,
            'min_is_coef':       min_is_coef,
            'frac_is_clipped':   frac_is_clipped,
        }
    ...
    def _update_critic(self, data: NStepReplayBufferSamples) -> Tuple[float, float]:
        ...
        with torch.no_grad():
            ...
            y = q_traj[:,0:1].clone()
            mean_bootstrap_q = q_traj[:, 0].mean().item() 
            ...
            for k in range(n):
                ...
                reward_term  = r_k * self.dt                              # ← NEW
                next_q_term  = gamma_dt * (1.0 - done_k) * q_kp1        # ← NEW
                delta_k = reward_term + next_q_term - q_k
 
                deltas_list.append(delta_k)                               # ← NEW
                reward_terms_list.append(reward_term)                     # ← NEW
                next_q_terms_list.append(next_q_term)                    # ← NEW
                q_k_terms_list.append(q_k)  
        ...
        with torch.no_grad():
            ...
            bellman_residual = mean_q - mean_td_target                      # ← NEW
 
            # δ_k breakdown
            delta_tensor      = torch.stack(deltas_list, dim=1)             # (B, n, 1)
            mean_delta        = delta_tensor.mean().item()                  # ← NEW
            mean_delta_pos    = delta_tensor.clamp(min=0).mean().item()     # ← NEW
            mean_delta_neg    = delta_tensor.clamp(max=0).mean().item()     # ← NEW
 
            # Component breakdown of δ_k
            mean_reward_term  = torch.stack(reward_terms_list, dim=1).mean().item()   # ← NEW
            mean_next_q_term  = torch.stack(next_q_terms_list, dim=1).mean().item()   # ← NEW
            mean_q_k_term     = torch.stack(q_k_terms_list,    dim=1).mean().item()   # ← NEW
 
            # IS coefficient stats (only valid when n > 1)
            if n > 1:
                mean_is_coef    = c_all.mean().item()                       # ← NEW
                min_is_coef     = c_all.min().item()                        # ← NEW
                frac_is_clipped = (c_all >= 1.0 - 1e-6).float().mean().item()  # ← NEW
            else:
                mean_is_coef = min_is_coef = frac_is_clipped = 1.0         # n=1: no IS needed
 
        return (float(np.mean(losses)), mean_q, critic_grad_norm, per_critic_means,
                mean_td_target, mean_bootstrap_q, bellman_residual,
                mean_delta, mean_delta_pos, mean_delta_neg,
                mean_reward_term, mean_next_q_term, mean_q_k_term,
                mean_is_coef, min_is_coef, frac_is_clipped)
    ...
    def _m_step(self, obs, action_samples, weights, b_mu, b_sigma) -> Tuple[float, float, float]:
        ...
        actor_grad_norms = []
        actor_grad_norms_P, actor_grad_norms_M, actor_grad_norms_S = [], [], []
        for _ in range(self.args.mstep_iteration_num):
            ...
            # --- component-wise gradient decomposition (P / M / S) ---
            # Total actor grad is exactly g_P + g_M + g_S (loss is additive; the
            # constant -eps terms have zero gradient). We build .grad from these
            # directly instead of loss.backward() to avoid double-counting.
            params = list(self.actor.parameters())
            g_P = torch.autograd.grad(nll, params,
                                      retain_graph=True, allow_unused=True)
            g_M = torch.autograd.grad(self.alpha_mu * kl_mu, params,
                                      retain_graph=True, allow_unused=True)
            g_S = torch.autograd.grad(self.alpha_sigma * kl_sigma, params,
                                      allow_unused=True)  # last grad: free the graph
 
            def _gnorm(gs):
                return sum((g.norm().item() ** 2) for g in gs if g is not None) ** 0.5
            preclip_P = _gnorm(g_P)
            preclip_M = _gnorm(g_M)
            preclip_S = _gnorm(g_S)
 
            # build the true total gradient and write it into .grad
            for p, a, b, c in zip(params, g_P, g_M, g_S):
                total = sum(x for x in (a, b, c) if x is not None)
                p.grad = total if isinstance(total, torch.Tensor) else None
 
            # clip, capturing the single scale factor applied to every grad
            if self.args.max_grad_norm > 0:
                total_norm = nn.utils.clip_grad_norm_(params, self.args.max_grad_norm)
                clip_coef = min(1.0, self.args.max_grad_norm / (total_norm.item() + 1e-6))
            else:
                clip_coef = 1.0
 
            # post-clip component norms = what is ACTUALLY applied in optimizer.step()
            actor_grad_norms_P.append(preclip_P * clip_coef)
            actor_grad_norms_M.append(preclip_M * clip_coef)
            actor_grad_norms_S.append(preclip_S * clip_coef)
        ...
        return (loss_p_val, kl_mu_val, kl_sigma_val, actor_entropy_val,
                float(np.mean(actor_grad_norms)),
                float(np.mean(actor_grad_norms_P)),
                float(np.mean(actor_grad_norms_M)),
                float(np.mean(actor_grad_norms_S)))
    ...
    def log_step(self, global_step, infos, rewards, metrics=None):
        ...
        if metrics is not None:
            row = {
                ...
                "actor_grad_norm_P": metrics.get('actor_grad_norm_P'),
                "actor_grad_norm_M": metrics.get('actor_grad_norm_M'),
                "actor_grad_norm_S": metrics.get('actor_grad_norm_S'),
                "actor_entropy": metrics.get('actor_entropy'),
                # AFTER:
                
                "mean_bootstrap_q": metrics.get('mean_bootstrap_q'),
                "bellman_residual": metrics.get('bellman_residual'),
                "mean_delta":       metrics.get('mean_delta'),
                "mean_delta_pos":   metrics.get('mean_delta_pos'),
                "mean_delta_neg":   metrics.get('mean_delta_neg'),
                "mean_reward_term": metrics.get('mean_reward_term'),
                "mean_next_q_term": metrics.get('mean_next_q_term'),
                "mean_q_k_term":    metrics.get('mean_q_k_term'),
                "mean_is_coef":     metrics.get('mean_is_coef'),
                "min_is_coef":      metrics.get('min_is_coef'),
                "frac_is_clipped":  metrics.get('frac_is_clipped'),
            }
